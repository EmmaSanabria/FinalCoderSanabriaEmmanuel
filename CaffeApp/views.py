...
class MessagesDelete(LoginRequiredMixin, DeleteView):
    ...
    success_url= '/CaffeApp/my-reservations/'    
...
def search_messages(request):
    ...
    if request.GET["data"]:
        ...
        messages = Messages.objects.filter(name= data)
        return render(request, "messages/search_messages.html", {"messages": messages })
    
    else:
        miFormulario = Messages()
        return render(request, 'messages/index_page.html',{'miFormulario': miFormulario})
    
def my_reservations(request):
    return render(request, "messages/my_reservation.html")

@method_decorator(staff_member_required(login_url='/no-admin/'), name='dispatch')
...
#Inicio del sector de Vistas No Admin

@method_decorator(login_required(login_url='login'), name='dispatch')
class ProductListNoad(LoginRequiredMixin, ListView):
    model= Product
    template_name= "no_admin/show_products_noad.html"
    context_object_name= "products"

@method_decorator(login_required(login_url='login'), name='dispatch')
class ProductDetailNoad(LoginRequiredMixin, DetailView):
    model= Product
    template_name= "no_admin/detail_product_noad.html"
    context_object_name= "product"

@method_decorator(login_required(login_url='login'), name='dispatch')
class StaffListNoad(LoginRequiredMixin, ListView):
    model= Staff
    template_name= "no_admin/show_staff_noad.html"
    context_object_name= "Staff"
 
@method_decorator(login_required(login_url='login'), name='dispatch')   
class StaffDetailNoad(LoginRequiredMixin, DetailView):
    model= Staff
    template_name= "no_admin/detail_staff_noad.html"
    context_object_name= "Staff"
    
@method_decorator(login_required(login_url='login'), name='dispatch')
class PromoListNoad(LoginRequiredMixin, ListView):
    model= Promo_menu
    template_name= "no_admin/show_promo_noad.html"
    context_object_name= "promo"

@method_decorator(login_required(login_url='login'), name='dispatch')
class PromoDetailNoad(LoginRequiredMixin, DetailView):
    model= Promo_menu
    template_name= "no_admin/detail_promo_noad.html"
    context_object_name= "promo"
#Final del sector de Vistas No Admin